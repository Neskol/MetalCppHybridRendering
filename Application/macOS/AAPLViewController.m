...
- (void)viewDidLoad
{
    ...
    _denoiseSwitch.state = NSControlStateValueOn;
    ...
    if (!_view.device)
    {
        ...
        _loadingLabel.stringValue = @"Metal is unavailable on this device.";
        return;
    }
        ...
        if ( (strongSelf = weakSelf) )
        {
            ...
            if (!strongSelf->_renderer)
            {
                dispatch_async(dispatch_get_main_queue(), ^(){
                    ...
                    if ( (innerStrongSelf = weakSelf) )
                    {
                        ...
                        innerStrongSelf->_loadingLabel.stringValue = @"Renderer failed initialization.";
                    }
                });
                return;
            }
            ...
            dispatch_async( dispatch_get_main_queue(), ^(){
                ...
                if ( (innerStrongSelf = weakSelf) )
                {
                    ...
                    [innerStrongSelf->_renderer setDenoisingEnabled:(innerStrongSelf->_denoiseSwitch.state == NSControlStateValueOn)];
...
- (IBAction)onDenoiseSwitchAction:(id)sender
{
    if ( sender == _denoiseSwitch )
    {
        [_renderer setDenoisingEnabled:(_denoiseSwitch.state == NSControlStateValueOn)];
    }
}
